...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Start the RMQTT Things demo environment",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  uv run scripts/demo-run.py                # Normal mode (simple output)
  uv run scripts/demo-run.py -v             # Verbose mode (detailed logs)
  uv run scripts/demo-run.py -vv            # Debug mode (very detailed)
  uv run scripts/demo-run.py --profile      # Show timing summary at end
  uv run scripts/demo-run.py -q             # Quiet mode (errors only)
        """
    )
