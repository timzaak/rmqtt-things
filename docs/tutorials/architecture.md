...
生产环境会用内网 CA 或 Let's Encrypt，但 IoT 设备证书不一样：设备数量多、需要批量签发、CN 里要塞 productId/deviceId。rcgen 是纯 Rust 的证书生成库，不需要装 OpenSSL，用一次性命令 `--generate-ca` 生成 CA 后，运行时只加载校验（BYO-CA：缺失即启动失败）。
...
启动时调用 `load_ca()` 只校验 `conf/ca.pem` 和 `conf/ca.key` 是否存在、是否有效，缺失或无效直接启动失败（BYO-CA）。CA 和服务端证书用一次性命令 `rmqtt-things --generate-ca` 生成（已存在时拒绝覆盖）。CA 默认有效期 100 年。服务端证书在 CA 生成后签发，CN 是配置里的 domain（支持通配符）。
