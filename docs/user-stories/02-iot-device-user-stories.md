...
> 详细业务规则见 [`docs/prd/core/support-multiple-device.md`](../prd/core/support-multiple-device.md)。设备只能读取自身 SN 命名空间下的出厂元数据。

### 故事 11：设备运行时读取自身出厂元数据 [US-DV-011]

**优先级**: P1

**【用户故事】**
**作为**：IoT Device（详见 [docs/user-stories/_roles.md](/_roles.md)）
**我希望**：上线后能读取自己的出厂元数据（含设备级元数据与子组件元数据），即便子组件仅部分到达
**从而**：设备能拿到自己的出厂配置（如整机序列号标签、子组件标定参数）生效，无需硬编码或本地重算

**【验收标准】**

**场景 1：设备读取到完整出厂元数据**
```gherkin
Given 设备的出厂元数据与子组件关联均已上报
When 设备上线后读取自身出厂元数据
Then 设备收到完整的出厂元数据（含设备级元数据与子组件元数据）
```

**场景 2：子组件部分到达时返回当前存在部分**
```gherkin
Given 设备的部分子组件元数据尚未上报
When 设备读取自身出厂元数据
Then 设备收到当前存在的部分元数据，平台不报错也不阻塞
And 设备可自行容忍缺失或稍后重读
```
