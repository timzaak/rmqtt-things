...
管理端认证是否启用取决于 Herald 是否配置，**这是有意的设计行为，不是缺陷**：

- **Herald 已配置（生产 / 受控部署）**：所有 `/api/admin/*` 路由挂载 Herald 认证与权限中间件。未携带有效凭据的请求返回未认证错误；已配置但 Herald 运行时不可用时返回 503 服务不可用，而非放行。
- **Herald 未配置（本地开发 / 独立部署 / 由网络层保护的环境）**：管理端路由**按设计不挂载认证中间件，直接放行**，便于本地开发和不依赖 Herald 的独立部署。此类环境的管理端访问安全由网络隔离（反向代理、内网 / VPC）保障。

> 配置判据：`backend/src/api/mod.rs` 中仅当 `config.herald` 与 `herald_client` 同时存在时挂载 Herald 中间件，否则不挂载。**生产部署必须配置 Herald 才能获得认证保护**；未配置即等同于"无认证"部署模式。
...
- rmqtt-things 为**单租户**内部系统，当前认证后仅将用户标识（`user_id`）注入请求上下文，**不按 realm 做业务数据隔离**。`realm` 是 Herald 侧的部署配置项，rmqtt-things 不消费它做多租户隔离
...
- 现有设备端 HMAC 认证机制完全不变
- Herald **已配置但运行时不可用**（网络故障 / Herald 宕机）时，管理端 API 应返回 503 服务不可用错误，而非放行。注意：此为"已配置但故障"的语义，与"Herald 未配置时按设计放行"（见上文《认证启用条件》）是两种不同情况
...
> **网络层前提**：OAuth 登录端点（`/api/auth/oauth/start`、`/api/auth/oauth/callback`）与管理端 API 一起部署在反向代理之后，由反代注入内网 IP 头（`X-Real-IP` / `X-Forwarded-For`）。浏览器通过反代访问这些端点，不直接从公网命中后端。
