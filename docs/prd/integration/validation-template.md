...
### 2.1 包含功能
- 校验模板 CRUD 管理（创建、列表查询、详情查看、编辑、删除）
...
7. 模板更新或状态变更时，系统自动清除该产品对应模板的 Schema 缓存（属性模板清属性缓存键，事件模板清事件缓存键）
...
- 运行时校验接口：属性上报与事件上报时均自动使用对应 Active 状态的 Schema 进行校验（属性取 `event=property` 模板；事件取 `event=<事件标识符>` 模板）。开启校验且无 Active 属性模板时，属性上报返回 400；事件上报在无 Active 事件模板时放行（不校验）
