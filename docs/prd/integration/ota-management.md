...
### 3.2 关键特性
- OTA 版本以 (product_id, key, version) 为唯一维度（软删除行 status=1 除外）；同一 (product_id, key) 下可存在多个不同 version 的记录（迁移 `20251020002800` 已将原 `(product_id, key)` 索引替换为含 version 的部分唯一索引）
- 版本号编码：major * 100000 + minor * 1000 + patch，前端以 x.y.z 格式交互；major / minor 各 ≤ 99、patch ≤ 999，超出将被拒绝
...
- 设备版本上报 payload 中 `version` 字段为**已编码整数**（major*100000+minor*1000+patch），非 x.y.z 字符串；管理端前端仍以 x.y.z 交互
