...
| 功能模块 | 状态 | 备注 |
|---------|------|------|
| OTA 版本 CRUD API | ✅ 已实现 | 后端完整，含软删除 |
| 设备版本上报与升级检测 | ✅ 已实现 | 自动检测并推送升级，含 10 分钟防抖 |
| 管理端文件上传 API | ✅ 已实现 | S3 预签名上传，目录白名单校验 |
| 固件下载 URL 生成 | ✅ 已实现 | S3 预签名下载 |
| 版本号格式化工具 | ✅ 已实现 | x.y.z 与整数互转（前端 version.ts） |
| OTA 版本列表前端页面 | ✅ 已实现 | 产品筛选、分页、版本格式化、删除确认 |
| OTA 版本创建前端页面 | ✅ 已实现 | 文件上传含 MD5 计算、版本校验、设备 ID 管理、未保存离开提示 |
| OTA 版本编辑前端页面 | ✅ 已实现 | 只读关键字段、文件重新上传、未保存离开提示 |
| OTA 版本详情前端页面 | ✅ 已实现 | 完整字段展示、设备 ID 列表、返回列表入口 |
