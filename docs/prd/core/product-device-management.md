...
- 设备注册/注销流程（设备通过 MQTT 自动接入）— 见 `device-auto-provisioning.md`
- 设备分组管理
- 设备批量操作
- 属性数据聚合/统计分析
...
- 设备详情页在同一页面内展示设备完整信息；页面信息架构（区域划分与操作语义）以 `docs/prd/core/device-detail-experience.md` 为准
...
- 设备详情页分区展示设备各类业务数据，区域划分与操作语义以 `docs/prd/core/device-detail-experience.md` 为准
- 各历史区域支持独立分页浏览
