...
| 功能模块 | 状态 | 备注 |
|---------|------|------|
| 产品 CRUD API | ✅ 已实现 | 后端完整，前端列表/创建/编辑页面已完成 |
| 设备属性上报 | ✅ 已实现 | 通过 RMQTT WebHook 回调处理 |
| 设备事件上报 | ✅ 已实现 | 通过 RMQTT WebHook 回调处理 |
| 设备连接/断开 | ✅ 已实现 | 通过 RMQTT WebHook 回调处理 |
| 属性命令下发 | ✅ 已实现 | 支持在线推送和离线缓存 |
| 属性命令状态追踪 | ✅ 已实现 | 设备回报后更新状态 |
| 属性 Schema 校验 | ✅ 已实现 | 基于校验模板，可配置开关 |
| 设备状态查询 API | ✅ 已实现 | 管理端 API 已完成 |
| 属性/事件历史查询 API | ✅ 已实现 | 管理端 API 已完成 |
| 前端 React Query hooks | ✅ 已实现 | useDevices、useProperties、useEvents 已封装 |
| 设备列表前端页面 | ⚠️ 待实现 | 路由已注册，hooks 已就绪，页面内容仅标题占位 |
| 设备详情前端页面 | ⚠️ 待实现 | 路由已注册，hooks 已就绪，页面内容仅标题占位 |
