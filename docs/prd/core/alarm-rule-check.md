...
改进基于现有 Rust 技术栈（dashmap + tokio + sqlx）实现，无需引入新依赖。
...
- 持续时间窗口的状态存储在内存（DashMap），进程重启后重新积累
...
- 进程重启后持续时间窗口和计数窗口状态丢失，正在计时中的规则需要重新积累 — 可接受，不需要持久化
...
- 现有 acknowledged 布尔字段平滑迁移到 status 状态机（现有 acknowledged=true 对应 Acknowledged 状态）
...
- **持续时间窗口状态存内存**：使用 DashMap 存储持续时间追踪状态，进程重启后重新积累
- **不引入新依赖**：所有改进基于现有 Rust 技术栈（dashmap + tokio + sqlx + serde_json）
