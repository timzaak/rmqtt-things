...
5. 按当前阶段 slot 串行调度相应 agent。每个 slot agent 必须通过 `Agent` tool 启动，`subagent_type` 按 Agent Dispatch Mapping 映射。传入 prompt 必须包含：设计文档相关节、上游 slot handoff（如有）、`.claude/guides/` 路径、Agent Output Contract 要求的字段列表。
   - prompt 必须引用 `.claude/protocols/task-check-rubric.md`，要求 agent 在返回前自检 P0/P1 规则。
   - prompt 必须引用 `.claude/protocols/task-phase-execution.md`，避免生成无法被 `/t-run` 执行的 item。
...
下一步: /t-task-check realm-user-rbac --phase backend
```

## 相关引用
- `.claude/protocols/task-state-contract.md`
- `.claude/protocols/task-phase-execution.md`
- [context-isolator.md](/.claude/skills/t-task/references/context-isolator.md)
- [phase-validator.md](/.claude/skills/t-task/references/phase-validator.md)
- [phase-index-generator.md](/.claude/skills/t-task/references/phase-index-generator.md)
- [compat-all-mode.md](/.claude/skills/t-task/examples/compat-all-mode.md)
- [frontend-blocked-by-backend.md](/.claude/skills/t-task/examples/frontend-blocked-by-backend.md)
- [phased-backend-success.md](/.claude/skills/t-task/examples/phased-backend-success.md)
- [error-response-template.md](/.claude/skills/t-task/templates/error-response-template.md)
- [phase-index-template.md](/.claude/skills/t-task/templates/phase-index-template.md)
