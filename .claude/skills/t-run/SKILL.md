...
- `/t-run` 的执行单元、slot 顺序、失败处理、所需上下文统一参考 `.claude/protocols/task-phase-execution.md`。
...
- 目标阶段必须是 supported phase，且存在于当前任务 active phases 中。
- 目标阶段必须已生成，且 `phases[phase].generated_at` 非空。
- 前置阶段依赖统一参考 `.claude/protocols/task-phase-execution.md`。
- 当前阶段目录必须存在。
- 当前阶段必须包含：
  - `index.md`
...
- 状态结构与聚合规则：`.claude/protocols/task-state-contract.md`
- phase/slot/item 执行规则：`.claude/protocols/task-phase-execution.md`

## Item Selection
按 `.claude/protocols/task-phase-execution.md` 选择可执行 item：
...
## State Transition
1. 读取状态并确定执行范围。
2. 依据 `.claude/protocols/task-state-contract.md` 与 `.claude/protocols/task-phase-execution.md` 校验状态与 DAG。
