...
4. 按需进入：
   ...
   - `.claude/guides/frontend/patterns.md`
...
规则：
- `.claude/guides/frontend/development.md` 是 frontend 事实型主规范
- `.claude/guides/frontend/patterns.md` 是项目批准的常用实现模式
...
- 命名与覆盖范围只看 `.claude/guides/frontend/testid-standards.md`
- 若可能影响 Demo 选择器，检查 `demo/e2e/` 与 `.claude/guides/demo/demo-debugging.md`
...
详细门禁以 `.claude/guides/frontend/validation.md` 和 `.claude/guides/frontend/quality.md` 为准。

## 实现约束

- 路由、目录、生成代码、Realm 约定以 `.claude/guides/frontend/development.md` 为准
- Query、Form、API、Tailwind 常用模式以 `.claude/guides/frontend/patterns.md` 为准
...
- 不引用不存在的文档段落或伪造行号
- 不绕过 `.claude/guides/frontend/index.md` 的导航关系
- 不在没有证据时凭印象重写项目模式
